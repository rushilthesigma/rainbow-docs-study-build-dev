...
export default function ShareDialog({ item, onClose, asPanel = false }) {
  ...
  const body = (
    <div className="flex flex-col gap-4">
        ...
        {selected ? (
          <div className="rounded-xl border border-white/[0.10] bg-white/[0.04] p-3 flex flex-col gap-3">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-blue-500/20 border border-blue-400/30 flex items-center justify-center text-[12px] font-bold text-blue-200">
                {(selected.displayName || selected.handle || '?').charAt(0).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-white/90 truncate">{selected.displayName || selected.handle}</p>
                {selected.handle && <p className="text-[11px] text-white/40 truncate">@{selected.handle}</p>}
              </div>
              <button
          ...
          <div>
              ...
              <div className="mt-2 flex flex-col gap-1 max-h-48 overflow-y-auto">
                {results.map((u) => (
                  <button
                    key={u.userId}
                    onClick={() => pick(u)}
                    className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/[0.06] text-left transition-colors"
                  >
            ...
            {noResults && (
              <p className="mt-2 text-[12px] text-white/40">No account found for “{query.trim()}”.</p>
            )}
          </div>
        )}
        ...
        <div className="border-t border-white/[0.08] pt-3">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-white/35 mb-2">People with access</p>
          {loadingOutgoing ? (
            <div className="flex items-center gap-2 text-[12px] text-white/40 py-1">
              <Loader2 size={13} className="animate-spin" /> Loading…
            </div>
          ) : outgoing.length === 0 ? (
            <p className="text-[12px] text-white/35 py-1">Not shared with anyone yet.</p>
          ) : (
            <div className="flex flex-col gap-1.5">
              {outgoing.map((s) => (
                <div key={s.id} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-white/[0.03]">
                  <div className="w-7 h-7 rounded-full bg-white/[0.08] flex items-center justify-center text-[11px] font-bold text-white/70">
                    {(s.recipientName || s.recipientHandle || '?').charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[12.5px] font-medium text-white/85 truncate">{s.recipientName || s.recipientHandle}</p>
                    <p className="text-[10.5px] text-white/35">{s.status === 'pending' ? 'Invited' : 'Accepted'}</p>
                  </div>
                  <button
                    ...
                    className="inline-flex items-center gap-1 px-2 py-1 rounded-md bg-white/[0.06] hover:bg-white/[0.12] text-[11px] font-semibold text-white/70 transition-colors disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    disabled={busyShareId === s.id}
                    title="Revoke access"
                    className="p-1 rounded text-white/25 hover:text-rose-400 transition-colors disabled:opacity-50"
                  >
...
function PermissionToggle({ value, onChange }) {
  const opt = (val, label, Icon) => (
    <button
      ...
      className={`inline-flex items-center gap-1 px-2.5 py-1.5 text-[12px] font-semibold transition-colors ${
        value === val ? 'bg-blue-500/25 text-blue-100' : 'text-white/50 hover:text-white/80'
      }`}
    >
      <Icon size={12} /> {label}
    </button>
  ...
  return (
    <div className="inline-flex rounded-lg border border-white/[0.10] overflow-hidden divide-x divide-white/[0.10]">
      {opt('view', 'View', Eye)}
